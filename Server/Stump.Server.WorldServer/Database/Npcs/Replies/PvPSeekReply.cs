...
namespace Stump.Server.WorldServer.Database.Npcs.Replies
{
    ...
    public class PvPSeekReply : NpcReply
    {
        ...
        public override bool Execute(Npc npc, Character character)
        {
            ...
            foreach (var contract in character.Inventory.GetItems(x => x.Template.Id == (int)ItemIdEnum.OrderofExecution))
                character.Inventory.RemoveItem(contract);

            var item = ItemManager.Instance.CreatePlayerItem(character, (int)ItemIdEnum.OrderofExecution, 25);
