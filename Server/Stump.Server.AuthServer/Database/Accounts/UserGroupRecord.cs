...
namespace Stump.Server.AuthServer.Database
{
      public static class UserGroupDefaultValues
    {
        ...
        public static async void CreateDefaultValues()
        {
            foreach (var role in from RoleEnum role in Enum.GetValues(typeof(RoleEnum))
                                 where role != RoleEnum.None
                                 where !UserGroupRecord.Table.Query("SELECT * FROM groups WHERE groups.Role = " + (int) role).Any()
                                 select role)
            {
                await UserGroupRecord.Table.InsertAsync(new UserGroupRecord
                {
                    Name = role.ToString(),
    ...
    [TableName("groups")]
    [Table("groups")]
    public class UserGroupRecord : DbObject<UserGroupRecord>, IAutoGeneratedRecord, ISaveIntercepter
    {
